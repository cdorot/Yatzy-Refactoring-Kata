...
class FullHouseTest extends AbstractRuleTest {
  ...
  public void
    ...
    placeDiceOnFullHouseRoll(expectedScore, dice1, dice2, dice3, dice4, dice5);
  }
  ...
  public void should_score_0_When_there_isnt_sequential_dice_and_dice_placed_onlarge_straight() {
    placeDiceOnFullHouseRoll(0, 2, 3, 4, 5, 6);
  }

  private void placeDiceOnFullHouseRoll(
      int expectedScore, int dice1, int dice2, int dice3, int dice4, int dice5) {
    assertEquals(
        expectedScore,
        new YatzyGame(dice1, dice2, dice3, dice4, dice5).fullHouse(),
        assertionMessage("full house", expectedScore, dice1, dice2, dice3, dice4, dice5));
  }
}
